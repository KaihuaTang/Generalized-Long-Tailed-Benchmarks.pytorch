...
class train_center_single():
    ...
    def run(self):
        # Start Training
        self.logger.info('=====> Start Center Loss with Single Env Training')
        ...
        for epoch in range(num_epoch):
            ...
            for step, (inputs, labels, _, indexs) in enumerate(env1_loader):
                iter_info_print = {}

                self.optimizer.zero_grad()

                # additional inputs
                inputs, labels, indexs = inputs.cuda(), labels.cuda(), indexs.cuda()
                add_inputs = {}
