...
class train_center_ldam_dual():
    def __init__(self, args, config, logger, eval=False):
        ...
        # get loss
        self.loss_ldam   = create_loss(logger, config, self.train_loader)
    ...
    def run(self):
        # Start Training
        self.logger.info('=====> Start Center Loss + LDAM loss with Dual Env Training')
        ...
        for epoch in range(num_epoch):
            ...
            assert self.training_opt['loss'] == 'LDAM'
            self.loss_ldam.set_weight(epoch)
            ...
            for step, ((inputs1, labels1, _, indexs1), (inputs2, labels2, _, indexs2)) in enumerate(zip(env1_loader, env2_loader)):
                ...
                # calculate loss
                loss_ce = self.loss_ldam(predictions, labels)
