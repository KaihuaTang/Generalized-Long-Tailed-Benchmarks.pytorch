...
class train_center_triple():
    ...
    def run(self):
        # Start Training
        self.logger.info('=====> Start Center Loss with Triple Env Training')

        # preprocess for each epoch
        env1_loader, env2_loader, env3_loader = self.train_loader
        assert len(env1_loader) == len(env2_loader)
        assert len(env2_loader) == len(env3_loader)
        ...
        for epoch in range(num_epoch):
            ...
            for step, ((inputs1, labels1, _, indexs1), (inputs2, labels2, _, indexs2), (inputs3, labels3, _, indexs3)) in enumerate(zip(env1_loader, env2_loader, env3_loader)):
                iter_info_print = {}

                self.optimizer.zero_grad()

                # additional inputs
                inputs = torch.cat([inputs1, inputs2, inputs3], dim=0).cuda()
                labels = torch.cat([labels1, labels2, labels3], dim=0).cuda()
                indexs = torch.cat([indexs1, indexs2, indexs3], dim=0).cuda()
                add_inputs = {}
            ...
            if self.algorithm_opt['always_update'] or (epoch in self.algorithm_opt['update_milestones']):
                ...
                if self.algorithm_opt['env_type'] in ('inter', 'intra', 'inter_intra'):
                    self.update_env_by_score(env1_loader, env2_loader, env3_loader, total_image)            
                else:
    ...
    def update_env_by_score(self, env1_loader, env2_loader, env3_loader, total_image):
        ...
        env3_score = torch.zeros(total_image).fill_(1.0)
        # inverse distribution
        if self.algorithm_opt['env_type'] in ('inter', 'inter_intra'):
            inter_weight2 = self.generate_inter_weight(all_scores, total_image, tg_scale=self.algorithm_opt['sample_scale'] // 2)
            env2_score = env2_score * inter_weight2
            inter_weight3 = self.generate_inter_weight(all_scores, total_image, tg_scale=self.algorithm_opt['sample_scale'])
            env3_score = env3_score * inter_weight3
        if self.algorithm_opt['env_type'] in ('intra', 'inter_intra'):
            intra_weight2 = self.generate_intra_weight(cat_socres, total_image, tg_scale=self.algorithm_opt['sample_scale'] // 2)
            env2_score = env2_score * intra_weight2
            intra_weight3 = self.generate_intra_weight(cat_socres, total_image, tg_scale=self.algorithm_opt['sample_scale'])
            env3_score = env3_score * intra_weight3

        env1_loader.sampler.set_parameter(env1_score)
        env2_loader.sampler.set_parameter(env2_score)
        env3_loader.sampler.set_parameter(env3_score)
