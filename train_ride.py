...
class train_ride():
    ...
    def run(self):
        # Start Training
        self.logger.info('=====> Start RIDE Training')
        ...
        for epoch in range(self.training_opt['num_epochs']):
            ...
            for step, (inputs, labels, _, _) in enumerate(self.train_loader):
                ...
                if self.training_opt['loss'] == 'RIDE':
                    ...
                    loss = self.loss_fc(predictions, labels, extra_info)
                    iter_info_print[self.training_opt['loss']] = loss.sum().item()
                else:
                    all_loss = []
                    for logit in all_logits:
                        all_loss.append(self.loss_fc(logit, labels))
                    loss = sum(all_loss)
                    for i, branch_loss in enumerate(all_loss):
