...
class train_center_ride_mixup():
    def __init__(self, args, config, logger, eval=False):
        ...
        if self.algorithm_opt['cos_loss']:
            self.loss_center = CenterCosLoss(num_classes=classifier_args['num_classes'], feat_dim=classifier_args['feat_dim'] * 3)
        else:
            self.loss_center = CenterLoss(num_classes=classifier_args['num_classes'], feat_dim=classifier_args['feat_dim'] * 3)
    ...
    def mixup_criterion(self, pred, y_a, y_b, lam, extra_info):
        return lam * self.loss_fc(pred, y_a, extra_info) + (1 - lam) * self.loss_fc(pred, y_b, extra_info)
    ...
    def run(self):
        # Start Training
        self.logger.info('=====> Start Center Loss and Mixup with RIDE backbone Training')
        ...
        for epoch in range(num_epoch):
            ...
            for step, ((inputs1, labels1, _, indexs1), (inputs2, labels2, _, indexs2)) in enumerate(zip(env1_loader, env2_loader)):
                ...
                if self.training_opt['loss'] == 'RIDE':
                    ...
                    loss_ce = self.mixup_criterion(predictions, labels_a, labels_b, lam, extra_info)
                    iter_info_print[self.training_opt['loss']] = loss_ce.sum().item()
                else:
                    ce_losses = []
                    for logit in all_logits:
                        ce_losses.append(lam * self.loss_fc(logit, labels_a) + (1 - lam) * self.loss_fc(logit, labels_b))
                    loss_ce = sum(ce_losses)
                    for i, branch_loss in enumerate(ce_losses):
                        iter_info_print[self.training_opt['loss'] + '_{}'.format(i)] = branch_loss.sum().item()                        

                # center loss
                self.center_optimizer.zero_grad()
                loss_ct = self.mixup_center_criterion(features.view(features.shape[0], -1), predictions, labels_a, labels_b, lam) * center_weight
